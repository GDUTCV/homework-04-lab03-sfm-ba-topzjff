...
def compute_ba_residuals(parameters: np.ndarray, intrinsics: np.ndarray, num_cameras: int, points2d: np.ndarray,
    ...
    """
    # 提取对应三维点
    selected_points_3d = points3d[points3d_idxs]

    # 将3d点进行齐次化并获得其结果的转置
    homo_3d_points = np.concatenate((selected_points_3d, np.ones((selected_points_3d.shape[0], 1))), axis=1)
    ...
    # 使用相应的投影矩阵重新投影二维点
    calculated_points_2d = np.einsum('ijk,ki->ij', P, homo_3d_points_T)
    calculated_points_2d /= calculated_points_2d[:, -1].reshape((calculated_points_2d.shape[0], 1))
    calculated_points_2d = calculated_points_2d[:, :-1]

    # 计算残差
    residuals = np.linalg.norm(points2d - calculated_points_2d, axis=1)
    """ END YOUR CODE HERE """
    return residuals
